  ...
  <div class="min-h-screen bg-white">
    ...
    <main v-else class="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div v-if="relatedPages.length > 0" class="mt-12">
        <h2 class="text-xl font-bold text-slate-900 mb-4">Related Tools</h2>
        <div class="grid md:grid-cols-3 gap-4">
          <NuxtLink
            v-for="related in relatedPages"
            :key="related.slug"
            :to="`/create/${related.slug}`"
            class="block p-4 bg-white rounded-lg border border-slate-200 hover:border-slate-300 hover:shadow-sm transition-all"
          >
            <h3 class="font-semibold text-slate-900 mb-1">{{ related.title }}</h3>
            <p class="text-sm text-slate-600">{{ related.description }}</p>
          </NuxtLink>
        </div>
      </div>
    </main>
  ...
  try {
    ...
    if (data?.related_pages && data.related_pages.length > 0) {
      const { data: related, error: relatedError } = await supabase
        .from('seo_pages')
        .select('slug, title, description, type')
        .in('slug', data.related_pages.slice(0, 3))
        .eq('published', true)

      if (!relatedError && related) {
        relatedPages.value = related
      }
    }
...
useHead(() => ({
  ...
  meta: [
    ...
    { property: 'og:url', content: `https://slate.ink/create/${route.params.slug}` },
  ...
  link: [
    { rel: 'canonical', href: `https://slate.ink/create/${route.params.slug}` }
  ],
